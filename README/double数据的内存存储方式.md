...
其中 尾数 隐含了 "该尾数的最高位 整数 1，和小数点"，也正因此原因，所以 尾数的范围是 [1, 2) 的一个小数
...
用阶码来表示 指数。阶码 的 最高位（左 第一位 是 阶码 的符号位，1 表示 正数，0 表示负数）

float、double 类型的 尾数是隐含了 整数1，和小数点位置，即尾数 xxx<sub>(2)</sub>，表示 1.xxx<sub>(2)</sub>
...
实际上这永远算不完！这就是著名的浮点数精度问题。所以直到加上前面的整数部分算够53位就行了（隐藏位技术：最高位整数部分的 1 和 小数点 不写入内存）。 
...
* 对 负数 而言，反码除符号位外（符号位保持不变），
    ...
    * 移码：先求出 补码，把符号位改成相反的(按 位 取反，其他保持不变)
* 移码：等于 补码，把符号改成相反的。
* 指数位(Exponent)：用于存储科学计数法中的指数数据，并且采用移位存储。采用 阶码 的 移码 来表示

**阶码** 是用 移码 表示的。

这里会有一个 127 的偏移量，相当于 阶码 的最高为 阶码值 的符号位：阶码 的最高位 为 0 时，表示的 指数 为 负数；阶码 的最高位 为 1 时，表示的 指数 为 正数，
* 阶码值为 127 时相当于 0；
* 阶码值小于 127 时为 负；
* 阶码值 大于 127 时为 正。


比如阶码：1000 0001 表示 指数 为 129 - 127 = 2，表示 真值 为 2<sup>2</sup> 。而阶码 01111110 则表示指数为 -2，真值为 2<sup>-1</sup>. 
...
float：符号位 1 bit，阶码 08 bit (固定偏移 7F)，尾数23 bit，有固定隐含位（整数位的1、小数点隐含位置）

double：符号位 1 bit，阶码 11 bit (固定偏移 3FF)，尾数 52 bit，有固定隐含位（整数位的1、小数点隐含位置）
...
对于单精度浮点数（float）来说，有1位符号位，指数位(阶码)共8位，尾数共23位。

指数 能够表示的指数范围为 [-128, 127] 。

尾数为23位。当尾数全 1 时再加上小数点前面的 1，

指数 取到最大正数127（8位，阶码的最高位是符号位，正数最大127，负数最小-128）,浮点数取得正数的最大值。

+1.1111111 11111111 11111111<sub>(2)</sub> * 2^127（1.后面23个1，由于尾数值的范围是 [1, 2)，其最高位总为 1，故只需存取小数部分，所以小数位是 23 位 1，即尾数部分能表示的最大值接近 2），约等于 `2 * 2^127 = 3.4 * 10^38`。

负值的最小值为即为最大正值 的相反数，即 `-(2 * 2^127) = -3.4 * 10^38`


Double 的计算与此类似，double 的符号位为 63 位，指数为 第62位 ~ 第52位，共11位。表示的范围是 [-1024, 1023]。尾数为 第51位 ~ 第0为。

表示的范围为 +1.1111 11111111 11111111 11111111 11111111 11111111 11111111<sub>(2)</sub> * 2^1023  （1.后面52个1）约等于 2 * 2^1023 = 2^1024，即 1.7 * 10^308。负数亦然。
