...
这种结构是一种科学计数法，用符号、指数(阶码) 和 尾数 来表示，底数定为2——即把一个浮点数表示为 尾数 乘以 2的指数次方 再添上符号，
...
比如：10000001 表示 指数 为 129 - 127 = 2,表示 真值 为 2<sup>2</sup> 。而 01111110 则表示 2<sup>-1</sup>. 
...
+1.1111111 11111111 11111111 * 2^127（1.后面23个1，由于尾数值的范围是 [1, 2)，其最高位总为 1，故只需存取小数部分，所以小数为是 23 位 1），约等于 2 * 2^127 = 3.4 * 10^38。

负值的最小值为即为最大正值取反，即 -3.4 * 10^38


Double 的计算与此类似，double 的符号位为 63 位，指数为 第62位 ~ 第52位，共11位。表示的范围是 [-1024, 1023]。尾数为 第51位 ~ 第0为。

表示的范围为 +1.1111 11111111 11111111 11111111 11111111 11111111 11111111 * 2^1023  （1.后面52个1）为 1.7 * 10^308。负数亦然。
